...
# 1) Supports 8 to 30 (or more) clients via --num_clients (alias: --num_langs).
# 2) Adds WER + accuracy-like metrics to server CSV log:
#    - val_wer
#    - val_word_correct_acc
#    - val_sent_acc
# 3) Logs tail means for 10%, 20%, 30% across clients for CER, WER, and accuracy.
# 4) Optional sweep for training fraction via --ft_fracs "0.10,0.20,0.30"
...
def bytes_of_parameters(p: fl.common.Parameters) -> int:
    # Uses Flower serialized payload size, matches what would be sent over the wire (no protocol overhead)
...
class HyperFedAvg(FedAvg):
    def __init__(
        ...
        **kwargs,
        ...
        # Superset schema: includes qfedavg-like system fields
        base_fields = [
            "round",
            "wall_time_s",
            "clients_fit",
    ...
    def configure_fit(self, server_round: int, parameters, client_manager):
        ...
        # Cache how many clients were selected and the payload size they receive
        ...
        if len(cfg) > 0:
            ...
            # For delta norms we need numeric ndarrays of what was sent
            try:
    ...
    def aggregate_fit(self, server_round, results, failures):
        # Compute fit side stats (loss, delta norms, upload/download bytes)
        ...
        # Download: server -> selected clients, Upload: selected clients -> server (successful only)
    ...
    def aggregate_evaluate(self, server_round, results, failures):
        # Per-round wall time (align with qfedavg-style logging)
        if self._round_start_ts is None:
            wall_s = float("nan")
        else:
            wall_s = float(time.time() - self._round_start_ts)

        # Pull fit-side stats computed in aggregate_fit
        ...
        # If no eval results, still log a row
        if not results:
        ...
        # Merge fit stats (payload_bytes, round_upload_bytes, round_download_bytes, fit losses, deltas)
        for k, v in fit_stats.items():
            row[k] = float(v)
...
def run_one(args, ft_frac_value: float, log_csv_path: str) -> None:
    ...
    # --num_clients overrides --num_langs.
    if args.num_clients is not None:a
    ...
    if args.langs.strip():
        ...
        if n_clients > 0:
            langs = discovered[: max(1, n_clients)]
        else:
            langs = discovered
    ...
    # Clamp clients_per_round if user set something too large.
    ...
    # Unbounded baseline overrides most caps, but still respects chosen client count.
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="Number of clients (languages) to use. Overrides --num_langs when set.",
    )
    ...
    ap.add_argument(
        ...
        type=str,
        default="",
        help="Optional comma-separated list of ft fractions to sweep, example: 0.10,0.20,0.30",
    )
    ...
    ap.add_argument("--num_cpus_per_client", type=float, default=3.0)
    ...
    ap.add_argument(
        ...
        type=str,
        default="0.30",
        help="Comma-separated tail fractions for tail metrics across clients.",
    )
    ...
    # If sweeping multiple runs, shut down Ray between runs to avoid state bleed.
    for k, ftv in enumerate(ft_list):
        out_csv = args.log_csv if len(ft_list) == 1 else _suffix_log_csv(args.log_csv, ftv)
        print(f"[run] ft_frac={ftv:.2f} log_csv={out_csv}")
