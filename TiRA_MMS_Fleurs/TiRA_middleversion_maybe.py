...
def _sample_indices(n: int, k: int, seed: int) -> np.ndarray:
    """Deterministic subsample of indices [0, n)."""
    if k <= 0 or k >= n:
...
def sign_agreement(a: np.ndarray, b: np.ndarray, eps: float = 1e-12, sample_k: int = 0, seed: int = 0) -> Tuple[float, int]:
    """
    Fraction of coordinates where sign(a) == sign(b), ignoring near-zeros.
    Returns (p_agree, n_used).
    """
    a = a.reshape(-1)
    b = b.reshape(-1)
    n = int(a.shape[0])
    idx = _sample_indices(n, int(sample_k), int(seed))
    aa = a[idx]
    bb = b[idx]
...
def split_update_pcgrad(
    ...
    """
    Two-stage routing:
      1) If sign-agreement >= sign_thr, merge dv fully into shared (tail residual is zero).
      2) Else, decompose dv into:
           dv_tail = projection onto ref (can be negative, captures conflict along ref)
           dv_shared = dv - dv_tail  (non-conflicting projection with dv_shared dot ref == 0 when dot < 0)
         If dot >= 0, route fully to shared.
    Returns (dv_shared, dv_tail, stats).
    """
    dv = dv.astype(np.float64, copy=False).reshape(-1)
    ref = ref.astype(np.float64, copy=False).reshape(-1)

    cos = cosine_sim(dv, ref, eps=eps)
    p_agree, n_used = sign_agreement(dv, ref, eps=eps, sample_k=sign_sample_k, seed=seed)

    ref_n2 = float(np.dot(ref, ref))
    dot = float(np.dot(dv, ref))
    stats = {
        "cos": float(cos),
        "p_agree": float(p_agree),
        "sign_used": float(n_used),
        "dot": float(dot),
        "ref_n2": float(ref_n2),
    }

    # Stage 1: sign-consistent, treat as compatible
    if n_used > 0 and p_agree >= float(sign_thr):
        dv_shared = dv.copy()
        dv_tail = np.zeros_like(dv)
        stats["route"] = 1.0
        stats["mode"] = 1.0
        return dv_shared, dv_tail, stats

    # Stage 2: projection based split
    if ref_n2 <= eps:
        dv_shared = np.zeros_like(dv)
        dv_tail = dv.copy()
        stats["route"] = 0.0
        stats["mode"] = 2.0
        return dv_shared, dv_tail, stats

    dv_par = (dot / (ref_n2 + eps)) * ref
    if dot < 0.0:
        # conflict along ref stays in tail head; remainder goes to shared
        dv_tail = dv_par
        dv_shared = dv - dv_tail
        stats["route"] = 0.0
        stats["mode"] = 3.0
        return dv_shared, dv_tail, stats
    else:
...
def _apply_flat_delta_to_idxs(global_nds: List[np.ndarray], idxs: List[int], flat_delta: np.ndarray) -> List[np.ndarray]:
    """Return nds where nds[j] = global_nds[j] + delta_slice for each j in idxs."""
    out = [a.copy() for a in global_nds]
    flat = flat_delta.reshape(-1)
    off = 0
    for j in idxs:
        shp = out[j].shape
        n = int(out[j].size)
        part = flat[off : off + n].reshape(shp).astype(out[j].dtype, copy=False)
        out[j] = out[j] + part
        off += n
    if off != int(flat.shape[0]):
        raise ValueError(f"Flat delta length mismatch: used={off} total={int(flat.shape[0])}")
    return out
...
class TARPFedAvg(FedAvg):
    def __init__(
        ...
        route_cos_thr: float,
        route_sign_thr: float,
        route_sign_sample_k: int,
        route_eps: float,
        ...
        **kwargs,
        ...
        self.route_sign_thr = float(route_sign_thr)
        self.route_sign_sample_k = int(route_sign_sample_k)
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        if global_nds is None:
            # Fallback: use first client's params as baseline
        ...
        tail_cos = []
        if ref_vec is None:
            # If no reference, keep tail in tail head
            kept_tail = tail_group[:]
            merged_tail = []
            tail_cos = [0.0 for _ in tail_group]
            sign_agree = [0.0 for _ in tail_group]
            sign_used = [0.0 for _ in tail_group]
            n_routed = 0
            n_conflict = 0
        else:
            sign_agree = []
            ...
            n_routed = 0
            n_conflict = 0
            # Build pseudo-updates for head-wise routing:
            # - merged_tail contributes to shared head (possibly projected)
            # - kept_tail contributes to tail head (only the conflicting component)
            for w, nds, lang_idx in tail_group:
                dv = _flatten_params([nds[i] - global_nds[i] for i in range(len(nds))], self._idx_tail_head)
                seed = int((server_round * 1000003 + int(lang_idx) * 9176) & 0xFFFFFFFF)
                dv_shared, dv_tail, st = split_update_pcgrad(
                    dv=dv,
                    ref=ref_vec,
                    sign_thr=self.route_sign_thr,
                    sign_sample_k=self.route_sign_sample_k,
                    eps=self.route_eps,
                    seed=seed,
                )
                tail_cos.append(float(st.get("cos", 0.0)))
                sign_agree.append(float(st.get("p_agree", 0.0)))
                sign_used.append(float(st.get("sign_used", 0.0)))

                if float(st.get("route", 0.0)) >= 0.5:
                    n_routed += 1
                if float(st.get("dot", 0.0)) < 0.0:
                    n_conflict += 1

                # Tail update contributes to shared head via dv_shared (full or projected)
                nds_shared = _apply_flat_delta_to_idxs(global_nds, self._idx_shared_head, dv_shared)
                merged_tail.append((w, nds_shared, lang_idx))

                # Conflicting component contributes to tail head via dv_tail
                if float(np.linalg.norm(dv_tail)) > self.route_eps:
                    nds_tail = _apply_flat_delta_to_idxs(global_nds, self._idx_tail_head, dv_tail)
                    kept_tail.append((w, nds_tail, lang_idx))
        self._round_cos_stats = {
            ...
            "mean_sign_agree": float(sum(sign_agree) / max(1, len(sign_agree))) if sign_agree else float("nan"),
            "min_sign_agree": float(min(sign_agree)) if sign_agree else float("nan"),
            "mean_sign_used": float(sum(sign_used) / max(1, len(sign_used))) if sign_used else float("nan"),
            "n_routed": float(n_routed),
            "n_conflict": float(n_conflict),
        }
        ...
        # Helper to aggregate a list of clients on specific indices
        ...
        # 1) Non-head parameters: FedAvg over everyone (shared + tail)
        ...
        # 2) Shared head: aggregate from shared_group plus merged_tail
        agg_on(self._idx_shared_head, shared_group + merged_tail)

        # 3) Tail head: aggregate only from kept_tail; if none, keep previous
        if kept_tail:
        ...
        # 4) Ownership fix for lang_emb rows: overwrite row i with that client's row i
    ...
    def aggregate_evaluate(self, server_round, results, failures):
        ...
        wall = float(time.time() - self._t0)

        
        # Update smoothed risk (EMA) and compute tail set for next round.
        # Risk is derived from evaluation signals to avoid using raw local training loss.
        risk_map: Dict[int, float] = {}
        if self.route_risk_metric == "loss":
            for idx, v in loss_map.items():
                if idx >= 0 and v is not None and not math.isnan(float(v)):
        ...
        if self._risk_ema and self.route_tail_frac > 0.0:
            ...
            items.sort(key=lambda x: x[1], reverse=True)  # higher risk is worse
        ...
        row: Dict[str, float] = {
            ...
            "mean_sign_agree": float(self._round_cos_stats.get("mean_sign_agree", float("nan"))),
            "min_sign_agree": float(self._round_cos_stats.get("min_sign_agree", float("nan"))),
            "mean_sign_used": float(self._round_cos_stats.get("mean_sign_used", float("nan"))),
            "n_routed": float(self._round_cos_stats.get("n_routed", float("nan"))),
            "n_conflict": float(self._round_cos_stats.get("n_conflict", float("nan"))),
        }
        ...
        print(
            ...
            f"tailSet={len(self._tail_lang_idxs)} merged={int(row['merged_tail_into_shared'])} "
...
def run_one(args, ft_frac_value: float, log_csv_path: str) -> None:
    ...
    strategy = TARPFedAvg(
        ...
        route_tail_frac=float(args.route_tail_frac),
        route_cos_thr=float(args.route_cos_thr),
        route_sign_thr=float(args.route_sign_thr),
        route_sign_sample_k=int(args.route_sign_sample_k),
        route_eps=float(args.route_eps),
...
def main():
    ...
    ap.add_argument("--route_cos_thr", type=float, default=0.0, help="(Legacy) Cosine threshold; kept for logging and fallback")
    ap.add_argument("--route_sign_thr", type=float, default=0.60, help="Stage-1 sign agreement threshold for routing")
    ap.add_argument("--route_sign_sample_k", type=int, default=8192, help="Number of coordinates to sample for sign agreement (0=all)")
    ap.add_argument("--route_eps", type=float, default=1e-12, help="Small epsilon for routing computations")
    ap.add_argument("--route_warmup_rounds", type=int, default=5, help="Rounds before enabling tail head usage")
    ...
    ap.add_argument("--route_risk_metric", type=str, default="cer", choices=["cer", "loss"], help="Risk signal for EMA tail set: cer (val_cer_mean) or loss (val_loss)")
